; TODO: add a Swing REPL implementation
; ORIGINAL from https://github.com/GeorgeJahad/debug-repl/
     ...
     `(let [current-ns# (ns-name *ns*)]        
        (let [counter# (inc-counter)
	        ...
	        (try
	         (binding [*level* (inc *level*), *ns* ~*ns*]
              ...
              #(do                  
	            ...
	            #(print (str "dr-" *level* "-" counter# 
	                      (if (or (nil? desc#) (not (string? desc#)) (empty? desc#)) 
	                        "" 
	                        (format " (%s)" desc#)) 
	                      " => "))
	            :eval eval-fn#
	         ...
	         (catch Exception e#
	           (do
	           (println (format "Exception caught: %s \nException == quit? %s" e# (= e# quit-dr-exception)))
	           (cond
	             (= e# quit-dr-exception)
